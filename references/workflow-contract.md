...
- **Inspect**：检查交付物、证据定位、硬约束、用户可见/后台边界和结束清理；向 CV Agent 交接时，额外检查所有被积木引用的经历是否具备组织/项目、角色关系和日期范围。
...
状态维护者只维护 commission 台账、检查记录、verdict、revision 与 state update，不代替生产者写内容，不把内部 reject/revise 信息展示给求职用户。用户明确结束时，销毁运行态台账中可关联的个人资料与 JD 信息；长期只保留此空白合同和完全合成的测试结论。
